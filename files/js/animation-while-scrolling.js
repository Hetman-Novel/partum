...
const animItems = document.querySelectorAll('.margin-left'); // animation
   ...
   function animOnScroll() {
      for (let index = 0; index < animItems.length; index++) {
         ...
         if ((pageYOffset > animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight)) {
            animItem.classList.remove('margin-left'); // animation-active
         } else {
            if (!animItem.classList.contains('animation-not-repeat')) {
               animItem.classList.add('margin-left'); // animation-active
            }
         }
      }
   }
